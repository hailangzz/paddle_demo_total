...
def train(model):
    ...
    for epoch in range(EPOCH_NUM):
        for batch_id, data in enumerate(train_loader()):
            ...
            # 计算损失
            loss = F.cross_entropy(predicts, labels) # 注：此处有soft_lable、use_softmax等参数可以满足labels,predicts shape不同时的可执行性[16, 1] [16, 10]
...
动态图在保存推理模型时，需要先将动态图模型转换为静态图写法，编译得到对应的模型结构再保存，而飞桨框架2.0版本推出paddle.jit.save和paddle.jit.load接口，
无需重新实现静态图网络结构，直接实现动态图模型转成静态图模型格式。paddle.jit.save接口会自动调用飞桨框架2.0推出的动态图转静态图功能，
使得用户可以做到使用动态图编程调试，自动转成静态图训练部署。

三种文件：
保存模型结构的*.pdmodel文件；
保存推理用参数的*.pdiparams文件和
保存兼容变量信息的*.pdiparams.info文件，
...
test_image, label = mnist_test[20]
# 获取读取到的图像的数字标签
