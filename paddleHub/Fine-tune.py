...
class DemoDataset(paddle.io.Dataset):
    def __init__(self, transforms, num_classes=4, mode='train'):
        # 数据集存放位置
        self.dataset_dir = r"D:\迅雷下载\AI数据集汇总\水蜜桃检测\peach\peach-classification"  # dataset_dir为数据集实际路径，需要填写全路径
...
trainer.train(peach_train, epochs=3, batch_size=16, eval_dataset=peach_validate, save_interval=1)
