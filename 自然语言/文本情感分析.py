...
def load_imdb(is_training):
    ...
    for label in ["pos", "neg"]:
        with tarfile.open(r"D:\迅雷下载\AI数据集汇总\自然语言数据集\IMDB的电影评论数据/aclImdb_v1.tar.gz") as tarf:
...
'''
网络定义
在讲解卷积神经网络的的章节，我们详细列出了每一种神经网络使用基础算子拼装的详细网络配置，但实际上对于一些常用的网络结构，飞桨框架提供了现成的中高层函数支持。下面用于情感分析的长短时记忆模型就使用
paddle.nn.LSTM
API实现。如果读者对使用基础算子拼装LSTM的内容感兴趣，可以查阅paddle.nn.LSTM
类的源代码。
'''
...
# 保存训练好的模型参数
paddle.save(sentiment_classifier.state_dict(), "D:\PycharmProgram\paddle_train_model\文本情感分析\{}.pdparams".format(model_name))
# 保存优化器参数，方便后续模型继续训练
paddle.save(optimizer.state_dict(), "D:\PycharmProgram\paddle_train_model\文本情感分析\{}.pdopt".format(model_name))