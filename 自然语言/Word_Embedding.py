...
# 读取text8数据
def load_text8():
    with open(r"D:\迅雷下载\AI数据集汇总\自然语言数据集\\text8_dataset\\text8.txt", "r") as f:
        corpus = f.read().strip("\n")
    f.close()
...
def subsampling(corpus, word2id_freq):
    ...
    def discard(word_id):
        return random.uniform(0, 1) < 1 - math.sqrt(
            1e-4 / word2id_freq[word_id] * len(corpus))  # 根据词语的频率来排除频率较高的词语不进入模型训练，防止过拟合
...
def build_data(corpus, word2id_dict, word2id_freq, max_window_size=3, negative_sample_num=4):
    ...
    for center_word_idx in range(len(corpus)):
        ...
        for positive_word in positive_word_candidates:
            ...
            dataset.append((center_word, positive_word, 1))  #元组数据
